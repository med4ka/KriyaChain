...
export default function OwnerAuthPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const endpoint = mode === "login" ? "/api/auth/owner/login" : "/api/auth/owner/register";
    ...
    <div className="min-h-screen bg-[#F8F7F4] text-[#4A2E1B] flex items-center justify-center px-4">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-serif font-bold">Portal Owner</h1>
          <p className="text-sm text-[#4A2E1B]/50 mt-1">{mode === "login" ? "Masuk ke akun Anda" : "Daftar sebagai kolektor"}</p>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-md border border-[#4A2E1B]/10 p-6 rounded-3xl shadow-xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" disabled={loading} className="w-full bg-amber-700 text-white font-bold py-3 rounded-xl hover:shadow-lg transition-all disabled:opacity-50 flex items-center justify-center gap-2">
